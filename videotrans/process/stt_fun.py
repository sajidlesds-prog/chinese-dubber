...
def faster_whisper(
    ...
    try:
        ...
        if compute_type in ['auto','default']:
            ...
            compute_type = settings.get('cuda_com_type', 'int8_float16') if is_cuda else settings.get('cpu_com_type', 'int8')
        ...
        if speech_timestamps:
            ...
            raws = [it for it in raws if it.get('end_time', 0) - it.get('start_time', 0) >= 500]
            if jianfan and raws:
                for it in raws:
                    it['text'] = zhconv.convert(it['text'], 'zh-hans')
    except BaseException:
