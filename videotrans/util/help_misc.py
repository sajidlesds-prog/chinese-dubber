...
def sanitize_path(original_path: str) -> str:
    """
    Copy file to temp dir if path contains non-ASCII or spaces.
    Returns safe path for FFmpeg processing.
    """
    ...
    if str(p).startswith(str(temp_dir)):
        return original_path
    
    has_non_ascii = any(ord(c) > 127 for c in p.name)
    has_space = ' ' in p.name
    
    if not has_non_ascii and not has_space:
        return original_path
    
    safe_name = p.name.encode('ascii', 'replace').decode('ascii').replace('?', '_')
    ...
    try:
        ...
        raise OSError(f"Failed to copy file to temp dir: {e}")
